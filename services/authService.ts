...
const wrongUsernameAndOrPassword = new Error(
  'Wrong username and/or password. Verify your entried data and try again...',
);

async function login(username: string, password: string): Promise<UserModel> {
  const encryptedPassword = toBase64(password);
  const signedUser = UserList.find(
    (user: UserModel) => user.username === username,
  );
  if (
    signedUser === undefined ||
    !signedUser?.id ||
    !isAValidPassword(signedUser.password, encryptedPassword)
  ) {
